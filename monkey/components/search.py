from monkey.core import search as monkey_search


class Search(object):
    """
    Provides a simple high-level interface to searching items in the App Engine Search API and
    utilizes the search helpers in :mod:`monkey.core.search`.
    """
    ...
    def search(self, index=None, query=None, limit=None, cursor=None, sort_field=None, sort_direction='asc', sort_default_value=None, options=None):
        ...
        Also takes care of setting pagination information if the :mod:`pagination component <monkey.components.pagination>` is present.

        See :func:`monkey.core.search.search` for more details.
        """

        index = index if index else self._get_index()
        query_string = query if query else self.controller.request.params.get('query', '')
        options = options if options else {}
        search_function = self.controller.meta.search_function if hasattr(self.controller.meta, 'search_function') else monkey_search.search
