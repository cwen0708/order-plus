...
class BackendUiMaterial(Controller):
    ...
    def root(self):
        ...
        if self.request.path.find("/admin") >= 0 and self.administrator.role.get().level >= 999:
            menus = self.util.get_menu("backend")
        else:
            menus = self.util.get_menu("dashboard")

        self.context["administrator_name"] = self.administrator.name
        self.context["administrator"] = self.administrator
    ...
    def setup_save(self):
        ...
        prohibited_actions = settings.get("administrator_prohibited_actions", u"")
    ...
    def login_json(self):
        ...
        if administrator is None:
            if has_record():
                return
        self.session["administrator_key"] = administrator.key
        self.context['data'] = {
            'is_login': 'true'
        }

    @route_with("/admin/logout")
    @route_with("/dashboard/logout")
    def logout(self):
