...
var w2utils = (function () {
	...
	var obj = {
		settings : {
            ...
            "fulldays": ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六","星期日"],
            "phrases": {
                "yesterday": "昨天",
                "none": "没有",
                "No items found": "没有找到項目",
                "Attach files by dragging and dropping or Click to Select": "附加文件通过拖放或单击以選中",
                "Remove": "移动",
                "Not a valid date": "不是一個有效的日期",
                "Required field": "必需填寫",
                "Saving...": "儲存中...",
                "Not an integer": "不是一個整数(int)",
                "Not a float": "不是一個浮點数(float)",
                "Not in money format": "不是貨幣格式",
                "Not a hex number": "不是一個十六進制数",
                "Not alpha-numeric": "不是字母或数字",
                "Not a valid email": "不是一個有效的Email地址",
                "Are you sure you want to delete selected records?": "你是想要刪除選中的記錄吗?",
                "Return data is not in JSON format. See console for more information.": "返回的資料不是JSON格式，更多訊息請在控制台中查看.",
                "Refreshing...": "重新整理...",
                "All Fields": "所有字段",
                "Multi Fields": "多個字段",
                ...
                "Yes": "是",
                "No": "不是",
                "Reload data in the list": "重新加载資料列表中",
                "Show/hide columns": "顯示/隱藏列",
                "Select Search Field": "選擇搜索字段",
                "Search...": "搜索中...",
                "Open Search Fields": "開放搜索字段",
                "is": "是(等于)",
                "begins with": "開始于",
                "contains": "包含",
                "ends with": "結束于",
                ...
                "Add New": "添加新的",
                "Delete selected records": "刪除選中的記錄",
                "Delete": "刪除",
                "Save changed records": "保存修改過的記錄",
                "Save": "保存",
                "Reset": "重置",
                "Search": "搜索",
                "Confirmation": "確認",
	...
	function lock (box, msg, showSpinner) {
		...
		setTimeout(function () {
			...
			setTimeout(function () {
				...
				if (showSpinner === true) msg = '<div class="w2ui-spinner" '+ (!msg ? 'style="width: 15px; height: 15px"' : '') +'></div>' + msg;
				mess.html(msg).css({
...
(function () {
	...
	w2grid.prototype = {
		...
		requestComplete: function(status, cmd, callBack) {
			var obj = this;
			this.unlock();
			setTimeout(function () { obj.status(w2utils.lang('伺服器查詢花了') + ' ' + ((new Date()).getTime() - obj.last.xhr_start)/1000 +' ' + w2utils.lang('秒')); }, 10);
			...
			if (status != 'error') {
				...
				obj.error('AJAX 錯誤. 查看控制台取得更多的資訊.');
			}
			// event after
			var url = (typeof this.url != 'object' ? this.url : this.url.get);
			if (!url) {
...
(function () {
	var w2form = function(options) {
		...
		this.msgRefresh		= w2utils.lang('載入中...');
		this.msgSaving		= w2utils.lang('儲存中...');
