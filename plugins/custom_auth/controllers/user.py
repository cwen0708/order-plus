import json
import logging
from monkey import wtforms
import webapp2

from monkey import auth, add_authorizations
from monkey import Controller, scaffold, settings, route_with, route
from monkey.components.flash_messages import FlashMessages
from monkey.core import mail
from google.appengine.api import users
from monkey.components import csrf
...
class User(Controller):
    class Meta:
        ...
        Model = models.SocialUserModel

    @route_with('/login')
    @route_with('/login.html')
        ...
        if self.request.method == "POST":
            ...
            try:
                ...
                if twitter_association_data is not None:
                    if models.SocialUserModel.check_unique(user.key, 'twitter', str(twitter_association_data['id'])):
                        social_user = models.SocialUserModel(
                            user=user.key,
                            provider='twitter',
                ...
                if fb_data is not None:
                    if models.SocialUserModel.check_unique(user.key, 'facebook', str(fb_data['id'])):
                        social_user = models.SocialUserModel(
                            user=user.key,
                            provider='facebook',
                            uid=str(fb_data['id']),
                ...
                if li_data is not None:
                    if models.SocialUserModel.check_unique(user.key, 'linkedin', str(li_data['id'])):
                        social_user = models.SocialUserModel(
                            user=user.key,
                            provider='linkedin',
                            uid=str(li_data['id']),
    ...
    def social_login_complete(self, provider_name):
        ...
        if provider_name == "twitter":
            ...
            if self.user():
                ...
                if models.SocialUserModel.check_unique(user_info.key, 'twitter', str(user_data['user_id'])):
                    social_user = models.SocialUserModel(
                        user=user_info.key,
                        provider='twitter',
                ...
                # login with twitter
                social_user = models.SocialUserModel.get_by_provider_and_uid('twitter', str(user_data['user_id']))
                if social_user:
        ...
        # github association
            ...
            if self.user():
                ...
                user_info = models.User.get_by_id(long(self.user_id()))
                if models.SocialUserModel.check_unique(user_info.key, 'github', str(user_data['login'])):
                    social_user = models.SocialUserModel(
                        user=user_info.key,
                        provider='github',
                ...
                social_user = models.SocialUserModel.get_by_provider_and_uid('github', str(user_data['login']))
                if social_user:
                    # Social user exists. Need authenticate related site account
                    user = social_user.user.get()
                    self.auth().set_session(self.auth().store.user_to_dict(user), remember=True)
        ...
        # facebook association
            ...
            if self.user():
                ...
                user_info = models.User.get_by_id(long(self.user_id()))
                if models.SocialUserModel.check_unique(user_info.key, 'facebook', str(user_data['id'])):
                    social_user = models.SocialUserModel(
                        user=user_info.key,
                        provider='facebook',
                        uid=str(user_data['id']),
                ...
                # login with Facebook
                social_user = models.SocialUserModel.get_by_provider_and_uid('facebook', str(user_data['id']))
                if social_user:
                    # Social user exists. Need authenticate related site account
                    user = social_user.user.get()
                    self.auth().set_session(self.auth().store.user_to_dict(user), remember=True)
        ...
        # association with linkedin
            ...
            if self.user():
                ...
                user_info = models.User.get_by_id(long(self.user_id()))
                if models.SocialUserModel.check_unique(user_info.key, 'linkedin', str(user_data['id'])):
                    social_user = models.SocialUserModel(
                        user=user_info.key,
                        provider='linkedin',
                        uid=str(user_data['id']),
                ...
                # login with Linkedin
                social_user = models.SocialUserModel.get_by_provider_and_uid('linkedin', str(user_data['id']))
                if social_user:
                    # Social user exists. Need authenticate related site account
                    user = social_user.user.get()
                    self.auth().set_session(self.auth().store.user_to_dict(user), remember=True)
        ...
        # google, myopenid, yahoo OpenID Providers
            ...
            if self.user():
                ...
                user_info = models.User.get_by_id(long(self.user_id()))
                if models.SocialUserModel.check_unique(user_info.key, provider_name, uid):
                    social_user = models.SocialUserModel(
                        user=user_info.key,
                ...
                social_user = models.SocialUserModel.get_by_provider_and_uid(provider_name, uid)
                if social_user:
    ...
    def create_account_from_social_provider(self, provider_name, uid, email=None, continue_url=None, user_data=None):
        ...
        """
        provider_display_name = settings.get('social_providers').get(provider_name).get('label')
        if models.SocialUserModel.check_unique_uid(provider_name, uid):
            # create user
            # Returns a tuple, where first value is BOOL.
            # If True ok, If False no new user is created
            ...
            if user_data:
                ...
                self.session[provider_name] = json.dumps(user_data)
            social_user.put()
            # authenticate user
            self.auth().set_session(self.auth().store.user_to_dict(user), remember=True)
            if settings.get('log_visit'):
    ...
    def remove_social_login(self, provider_name):
        ...
        if len(user.get_social_providers_info()['used']) > 1:
            social_user = models.SocialUserModel.get_by_user_and_provider(user.key, provider_name)
            if social_user:
